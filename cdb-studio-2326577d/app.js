...
function syncControls(){
  ...
  if(isText&&el){el.font=normalizeFontName(el.font);if(el.font==='Anton'||el.font==='Nickel Gothic v3')el.weight=400;$('fontSelect').value=el.font;$('weightRange').disabled=el.font==='Anton'||el.font==='Nickel Gothic v3';$('sizeRange').value=el.size;$('weightRange').value=el.weight;$('widthRange').value=el.width;$('lineRange').value=Math.round(el.lineHeight*100);$('colorInput').value=el.color;if($('alignSelect'))$('alignSelect').value=el.align||model.layout?.align||'left';if($('maxWidthRange'))$('maxWidthRange').value=Math.round(el.maxWidth||model.layout?.maxWidth||820);}
...
$('fontSelect').addEventListener('change',e=>{const el=currentElement();if(!el)return;el.font=normalizeFontName(e.target.value);if(el.font==='Anton'||el.font==='Nickel Gothic v3')el.weight=400;renderCanvasOnly();syncControls();markDirty();commitHistory();renderFilmstrip();});
...
$('exportSvgBtn').addEventListener('click',()=>withQualityGuard('esportare lo SVG',()=>guardedOperation('export-svg','Esportazione SVG Affinity',[$('exportSvgBtn')],async()=>{showToast('Preparo lo SVG per Affinity…');const [fontDataUrl,logoMarkup]=await Promise.all([loadAffinityFontDataUrl(),loadAffinityLogoMarkup()]),svg=buildAffinitySvg(currentSlide(),{suffix:'export',fontDataUrl,logoMarkup});downloadBlob(new Blob([svg],{type:'image/svg+xml;charset=utf-8'}),`${slug(project.name)}-${currentIndex+1}-affinity.svg`);showToast(fontDataUrl&&logoMarkup?'SVG Affinity pronto':'SVG pronto · alcuni asset non incorporati');},'SVG non creato. Riprova dopo aver ricaricato l’app.')));
...
async function initialize(){
  ...
  if('serviceWorker'in navigator&&location.protocol.startsWith('http'))navigator.serviceWorker.register('./sw.js?v=0.34.0').then(r=>r.update()).catch(error=>diagnosticLog('warn','service-worker','Registrazione non riuscita',error));
  try{logoData=await fileToDataUrl('assets/logo.svg');}catch(e){console.warn(e);try{logoData=await fileToDataUrl('assets/logo.png');}catch(fallbackError){console.warn(fallbackError);}}
