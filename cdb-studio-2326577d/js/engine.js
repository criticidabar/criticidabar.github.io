...
function normalizeFontName(value){const name=String(value||'').trim();if(!name)return 'Nickel Gothic v3';if(/nickel\s*gothic/i.test(name))return 'Nickel Gothic v3';return ['Anton','Anybody','Archivo','Saira'].includes(name)?name:'Nickel Gothic v3';}
function normalizeTextStyle(value={}){const out={...value,font:normalizeFontName(value.font)};if(out.font==='Anton'||out.font==='Nickel Gothic v3')out.weight=400;return out;}
function textDefaults(overrides={}){ return normalizeTextStyle({text:'',font:'Nickel Gothic v3',size:88,weight:400,width:90,lineHeight:.94,color:COLORS.cream,align:null,maxWidth:null,...transformDefaults(),...overrides}); }
function freeTextDefaults(overrides={}){return normalizeTextStyle({id:uid(),x:540,y:675,text:'NUOVO TESTO',font:'Nickel Gothic v3',size:72,weight:400,width:90,lineHeight:1,color:COLORS.orange,align:'center',maxWidth:820,...transformDefaults(),...overrides});}
function normalizeFreeText(value,index=0){const mapColor={'%%BLUE%%':COLORS.blue,'%%ORANGE%%':COLORS.orange,'%%CREAM%%':COLORS.cream};return freeTextDefaults({id:value?.id||uid(),text:value?.text??value?.t??'NUOVO TESTO',x:Number(value?.x??540),y:Number(value?.y??(600+index*90)),dx:Number(value?.dx||0),dy:Number(value?.dy||0),scale:Number(value?.scale??value?.s??1),rotation:Number(value?.rotation||0),size:Number(value?.size??value?.fs??72),color:mapColor[value?.color??value?.c]||value?.color||value?.c||COLORS.orange,font:normalizeFontName(value?.font||value?.f||'Nickel Gothic v3'),weight:Number(value?.weight||400),width:Number(value?.width||90),lineHeight:Number(value?.lineHeight||1),align:value?.align||'center',maxWidth:Number(value?.maxWidth||820)});}
...
function newProject(){
  ...
  return {version:'0.34.0',name:'Nuovo carosello',showNumbers:false,snapGuides:true,automationPreset:'auto',editorial:{preset:'auto'},slides:[a,b,c]};
}
...
function migrateLegacyProject(parsed){
  if(parsed?.client?.st && Array.isArray(parsed.slides)){
    ...
    return {version:'0.34.0',name:parsed.slides[0]?.postTitle||'Progetto importato',showNumbers:Boolean(parsed.client.globalNum),snapGuides:true,slides};
  }
  return parsed;
}
...
async function loadState(){
  try{
    ...
    saved.version='0.34.0';saved.slides=(saved.slides||[]).map(normalizeSlide);
