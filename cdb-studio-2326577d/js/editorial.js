...
function contentJsonProject(parsed,fileName=''){
  ...
  return {version:'0.34.0',name:String(root.name||root.title||fallbackName),showNumbers:explicitNumbers,snapGuides:true,automationPreset:resolvedPreset,editorial:{preset:resolvedPreset,tone:presetConfig(resolvedPreset).tone||'critical',density:presetConfig(resolvedPreset).density||'medium'},contentImport:{format:Array.isArray(parsed)?'array':'content-json',importedAt:new Date().toISOString()},slides};
}
