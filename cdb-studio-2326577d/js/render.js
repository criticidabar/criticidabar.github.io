...
function layoutFor(slide){
  ...
  const layouts={
    ...
    'classic-cover':{image:{x:0,y:810,w:W,h:540,cx:540,cy:1080},title:{x:540,y:350,maxWidth:920,align:'center',cx:540,cy:480},subtitle:{x:540,y:670,maxWidth:820,align:'center',cx:540,cy:690},logo:{x:400,y:34,w:280,h:200,cx:540,cy:134}},
    'classic-body':{image:{x:40,y:850,w:1000,h:460,cx:540,cy:1080,rx:18},title:{x:540,y:350,maxWidth:900,align:'center',cx:540,cy:490},subtitle:{x:540,y:680,maxWidth:850,align:'center',cx:540,cy:700},logo:{x:380,y:35,w:320,h:229,cx:540,cy:149}},
...
function affinityLayerName(key){
  if(key==='title')return'Titolo';if(key==='subtitle')return'Testo';if(key==='logo')return'Logo';if(key==='image')return'Immagine';if(key==='number')return'Numero slide';
  if(key===TEMPLATE_BACK_KEY)return'Sfondo';if(key===TEMPLATE_FRONT_KEY)return'Grafica';if(key===TEMPLATE_AUX_KEY)return'Testi template';
  if(key?.startsWith('free:'))return'Testo libero';if(key?.startsWith('overlay:'))return'Immagine libera';if(key?.startsWith('shape:'))return'Forma template';return'Elemento';
}
function affinityId(key,index=0){return`${affinityLayerName(key).normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-zA-Z0-9]+/g,'-')}-${index+1}`;}
function affinityTextSvg(slide,key,style,layout,index=0){
  if(!layout)return'';
  const lines=wrapLines(style.text,style,layout.maxWidth),anchor=layout.align==='center'?'middle':layout.align==='right'?'end':'start';
  const tspans=lines.map((line,i)=>`<tspan x="${layout.x}" dy="${i===0?0:style.size*style.lineHeight}">${esc(line)}</tspan>`).join('');
  const transform=`${transformAttr(style,layout.cx,layout.cy,style.rotation||0)} translate(${layout.x} 0) scale(${(style.width||100)/100} 1) translate(${-layout.x} 0)`;
  const name=affinityLayerName(key),family=String(normalizeFontName(style.font)||'Nickel Gothic v3').replace(/["']/g,'');
  return `<text id="${affinityId(key,index)}" data-name="${name}" aria-label="${name}" data-element="${key}" x="${layout.x}" y="${layout.y}" text-anchor="${anchor}" fill="${style.color}" opacity="${layerOpacity(slide,key)}" transform="${transform}" font-family="${esc(family)}" font-size="${style.size}" font-weight="${style.weight||400}">${tspans}</text>`;
}
function affinityImageSvg(slide,layout,clipId,index=0){
  if(!slide.image.src||!layout.w||!layout.h)return'';const p=cropPlacement(slide.image,layout),name=affinityLayerName('image');
  return `<image id="${affinityId('image',index)}" data-name="${name}" aria-label="${name}" data-element="image" href="${esc(slide.image.src)}" x="${p.x}" y="${p.y}" width="${p.w}" height="${p.h}" preserveAspectRatio="none" clip-path="url(#${clipId})" opacity="${layerOpacity(slide,'image')}" transform="${transformAttr(slide.image,layout.cx,layout.cy,(layout.rotation||0)+(slide.image.rotation||0))}"/>`;
}
function affinityLogoSvg(slide,layout,index=0,vectorMarkup=''){
  if(!layout.w||!layout.h)return'';const name=affinityLayerName('logo'),common=`id="${affinityId('logo',index)}" data-name="${name}" aria-label="${name}" data-element="logo" opacity="${layerOpacity(slide,'logo')}" transform="${transformAttr(slide.logo,layout.cx,layout.cy,slide.logo.rotation||0)}"`;
  if(vectorMarkup)return `<g ${common}><g transform="translate(${layout.x} ${layout.y}) scale(${layout.w/490} ${layout.h/349})">${vectorMarkup}</g></g>`;
  return `<image ${common} href="${logoData||'assets/logo.png'}" x="${layout.x}" y="${layout.y}" width="${layout.w}" height="${layout.h}" preserveAspectRatio="xMidYMid meet"/>`;
}
...
function buildAffinitySvg(slide,{suffix='',fontDataUrl='',logoMarkup=''}={}){
  ensureLayerModel(slide);const layout=layoutFor(slide),id=`a${slide.id}${suffix}`.replace(/[^a-zA-Z0-9]/g,''),clipId=`clip${id}`;
  const fontFace=fontDataUrl?`<style>@font-face{font-family:'Nickel Gothic v3';font-style:normal;font-weight:400;src:url('${fontDataUrl}') format('opentype')}</style>`:'';
  const defs=`<defs>${fontFace}<clipPath id="${clipId}"><rect x="${layout.image.x}" y="${layout.image.y}" width="${layout.image.w}" height="${layout.image.h}" rx="${layout.image.rx||0}"/></clipPath></defs>`;
  const back=layerVisible(slide,TEMPLATE_BACK_KEY)?`<g id="Sfondo-1" data-name="Sfondo" data-element="${TEMPLATE_BACK_KEY}" opacity="${layerOpacity(slide,TEMPLATE_BACK_KEY)}">${baseAndOverlaySvg(slide)}</g>`:'';
  const stack=slide.layerOrder.map((key,index)=>{
    if(!layerVisible(slide,key))return'';
    if(key==='title')return affinityTextSvg(slide,key,slide.title,layoutForElement(slide,key),index);
    if(key==='subtitle')return affinityTextSvg(slide,key,slide.subtitle,layoutForElement(slide,key),index);
    if(key.startsWith('free:')){const ft=(slide.freeTexts||[]).find(x=>`free:${x.id}`===key);return ft?affinityTextSvg(slide,key,ft,layoutForElement(slide,key),index):'';}
    if(key==='image')return affinityImageSvg(slide,layoutForElement(slide,key),clipId,index);
    if(key==='logo')return affinityLogoSvg(slide,layoutFor(slide).logo,index,logoMarkup);
    const body=renderElementLayerSvg(slide,key,clipId,id);return body?`<g id="${affinityId(key,index)}" data-name="${affinityLayerName(key)}">${body}</g>`:'';
  }).join('');
  return `<svg xmlns="${SVG_NS}" viewBox="0 0 1080 1350" width="1080" height="1350">${defs}${back}${stack}</svg>`;
}
